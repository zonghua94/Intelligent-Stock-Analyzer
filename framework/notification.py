...
from utils.results import AnalysisResult, BotMessage
from utils.config import NotificationArgs
from utils.formatters import format_feishu_markdown
...
class NotificationService:
    ...
    def __init__(self, args: NotificationArgs, source_message: Optional[BotMessage] = None):
        """
        初始化通知服务
        ...
        # 各渠道的 Webhook URL
        self._wechat_url = self.args.wechat_webhook_url
        self._feishu_url = self.args.feishu_webhook_url

        # 微信消息类型配置
        self._wechat_msg_type = self.args.wechat_msg_type
        # Telegram 配置
        self._telegram_config = {
            'bot_token': self.args.telegram_bot_token,
            'chat_id': self.args.telegram_chat_id,
            'message_thread_id': self.args.telegram_message_thread_id,
        }
        
        # 邮件配置
        self._email_config = {
            'sender': self.args.email_sender,
            'sender_name': self.args.email_sender_name,
            'password': self.args.email_password,
            'receivers': self.args.email_receivers or ([self.args.email_sender] if self.args.email_sender else []),
        }
        
        # Pushover 配置
        self._pushover_config = {
            'user_key': self.args.pushover_user_key,
            'api_token': self.args.pushover_api_token,
        }

        # PushPlus 配置
        self._pushplus_token = self.args.pushplus_token
       
        # Server酱3 配置
        self._serverchan3_sendkey = self.args.serverchan3_sendkey

        # 自定义 Webhook 配置
        self._custom_webhook_urls = self.args.custom_webhook_urls
        self._custom_webhook_bearer_token = self.args.custom_webhook_bearer_token
        
        # Discord 配置
        self._discord_config = {
            'bot_token': self.args.discord_bot_token,
            'channel_id': self.args.discord_main_channel_id,
            'webhook_url': self.args.discord_webhook_url,
        }

        self._astrbot_config = {
            'astrbot_url': self.args.astrbot_url,
            'astrbot_token': self.args.astrbot_token,
        }
        
        # 消息长度限制（字节）
        self._feishu_max_bytes = self.args.feishu_max_bytes
        self._wechat_max_bytes = self.args.wechat_max_bytes
    ...
    def _send_feishu_stream_reply(self, chat_id: str, content: str) -> bool:
        ...
        try:
            ...
            app_id = self.args.feishu_app_id
            app_secret = self.args.feishu_app_secret
            ...
            # 飞书文本消息有长度限制，需要分批发送
            max_bytes = self.args.feishu_max_bytes
            content_bytes = len(content.encode('utf-8'))
