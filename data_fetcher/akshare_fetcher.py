...
class AkshareFetcher(BaseFetcher):
    ...
    def __init__(self, args: FetcherArgs, sleep_min: float = 2.0, sleep_max: float = 5.0):
        """
        初始化 AkshareFetcher
    ...
    def get_all_realtime_quote(self, source: str = "em") -> Optional[pd.DataFrame]:
        """
        获取实时行情数据（支持多数据源）
        ...
        return self._get_all_stock_realtime_quote_em()

    def get_income_data(self, stock: str, source: str = "em"):
        # 检查熔断器状态
        circuit_breaker = get_realtime_circuit_breaker()
        source_key = f"akshare_{source}"
        ...
        if not circuit_breaker.is_available(source_key):
            logger.warning(f"[熔断] 数据源 {source_key} 处于熔断状态，跳过")
            return None
        return self._get_income_data_em(stock)

    def _get_income_data_em(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        import akshare as ak
        circuit_breaker = get_realtime_circuit_breaker()
        ...
        try:
            ...
            #    logger.debug(f"[缓存命中] A股实时行情(东财) - 缓存年龄 {cache_age}s/{_realtime_cache['ttl']}s")
                ...
                for attempt in range(1, 3):
                    try:
                        ...
                        logger.info(f"[API调用] ak.stock_financial_abstract() 获取A股实时行情... (attempt {attempt}/2)")
                        import time as _time
                        api_start = _time.time()
                        ...
                        api_elapsed = _time.time() - api_start
                        logger.info(f"[API返回] ak.stock_financial_abstract 成功: 返回 {len(df)} 条信息, 耗时 {api_elapsed:.2f}s")
                        circuit_breaker.record_success(source_key)
                        break
                    except Exception as e:
                        last_error = e
                        logger.warning(f"[API错误] ak.stock_financial_abstract 获取失败 (attempt {attempt}/2): {e}")
                        time.sleep(min(2 ** attempt, 5))

                # 更新缓存：成功缓存数据；失败也缓存空数据，避免同一轮任务对同一接口反复请求
                #if df is None:
                #    logger.error(f"[API错误] ak.stock_financial_abstract 最终失败: {last_error}")
            ...
            if not df.empty:
                income_col = '营业总收入'
                income_inc_col = '营业总收入增长率'
                profit_col = '净利润'
                profit_inc_col = '归属母公司净利润增长率'
                df = df[df['指标'].isin([income_col, income_inc_col, profit_col, profit_inc_col]) & (df['选项'] == '成长能力')]
                result = {
                        "code": stock_code,
                        "income": df[df['指标'] == income_col].iloc[:, 2].tolist()[0],
                        "income_inc": df[df['指标'] == income_inc_col].iloc[:, 2].tolist()[0],
                        "profit": df[df['指标'] == profit_col].iloc[:, 2].tolist()[0],
                        "profit_inc": df[df['指标'] == profit_inc_col].iloc[:, 2].tolist()[0]
                    }
                return result
            else:
                return None
            ...
            logger.error(f"[API错误] 获取 {stock_code} 季度信息失败: {e}")
    ...
    def get_batch_income_data(self, date: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.info(f"[API调用] stock_yjbb_em(date='{date}') 获取全市场业绩...")
            import time as _time
            api_start = _time.time()
            ...
            try:
                ...
                df = stock_yjbb_em(date=date)
            except (ImportError, AttributeError):
                import akshare as ak
                df = ak.stock_yjbb_em(date=date)

            api_elapsed = _time.time() - api_start
            logger.info(f"[API返回] stock_yjbb_em 成功: {len(df)} 条, 耗时 {api_elapsed:.2f}s")
            circuit_breaker.record_success(source_key)
            ...
            if inc_col is None:
                for col in df.columns:
                    if '营业' in col and '同比' in col and '增长' in col:
                        ...
                        break
            if inc_col is None:
                logger.error(f"[akshare] 未找到营业收入同比增长列，可用列: {df.columns.tolist()}")
                return None
            ...
            logger.error(f"[API错误] 获取全市场业绩(akshare)失败: {e}")
            circuit_breaker.record_failure(source_key, str(e))
            return None
    ...
    def _get_stock_industry_info_em(self, stock_code):
        ...
        try:
            ...
            api_elapsed = _time.time() - api_start
            logger.info(f"[API返回] ak.stock_individual_info_em({stock_code}) 成功: {len(df)} 条, 耗时 {api_elapsed:.2f}s")
            circuit_breaker.record_success(source_key)
            ...
            logger.error(f"[API错误] 获取 {stock_code} 行业信息(东财)失败: {e}")
            circuit_breaker.record_failure(source_key, str(e))
            return None
    ...
    def get_sector_stock_mapping(self) -> Optional[Dict]:
        ...
        Returns:
            {
                'sectors': [{'name': str, 'label': str, 'change_pct': float}, ...],
                'stock_to_sector': {stock_code: sector_name, ...}
            }
            失败返回 None
        """
        import akshare as ak
        ...
        try:
            ...
            logger.warning(f"[Akshare] 获取板块映射失败: {e}")
            return None
