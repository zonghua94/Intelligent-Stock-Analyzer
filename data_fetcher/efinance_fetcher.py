...
class EfinanceFetcher(BaseFetcher):
    ...
    def __init__(self, args: FetcherArgs, sleep_min: float = 1.5, sleep_max: float = 3.0):
        """
        初始化 EfinanceFetcher
    ...
    def get_all_realtime_quote(self) -> Optional[pd.DataFrame]:
        """
        获取实时行情数据
        ...
        try:
            ...
            logger.error(f"[API错误] 获取汇总实时行情(efinance)失败: {e}")
